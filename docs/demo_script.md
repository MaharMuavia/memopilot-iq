...
| 0:35–1:05 | Send "Design the backend architecture." + Trace panel | "Next session I ask for an architecture. The **Memory Trace** shows exactly which memories were retrieved, their scores, and that the critical rule was always injected — within a 2,500-token budget." |
...
| 2:05–2:30 | Evaluation tab → Run benchmark | "The Evaluation Dashboard runs 6 scenarios: memory agent vs no-memory baseline. Higher accuracy, strong recall@5, zero outdated errors, ~60% token savings." |

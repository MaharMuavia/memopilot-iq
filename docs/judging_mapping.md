...
| **Technical Depth** | 30% | Modular async FastAPI; hybrid (dense+sparse) retrieval; context budget manager; evaluation benchmark; dual LOCAL/ALIBABA modes with safe fallback; error handling; 17 passing tests. | `memory/`, `eval/benchmark.py`, `tests/` |
...
- ✅ Alibaba Cloud deployment (Tablestore + OSS + ECS/FC) with proof doc
...
- ✅ Runs consistently and matches the description (offline fallback guarantees it)
